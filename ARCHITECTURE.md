...
**One call per deal (default, `llm.one_call_per_deal`).** The deterministic
engine still runs first (fast, local: grounding evidence, positional comps/cap
tables, derived fields), but the LLM second pass is the primary field extractor
and runs as ONE `claude -p` call per deal-period over the COMBINED payload of
all that deal's documents — not the per-memo, per-band, per-tier fan-out.
`run._build_deal_groups` groups the assembled `(item, memo)` pairs exactly like
the multi-doc merge (`merge_key`, non-merge items standalone), picks the primary,
and hands each group to `escalate.process_deals`. `payload.assemble_deal_payload`
concatenates every document's pages under a single GLOBAL page index (each block
labelled with its source document + that document's own page number, so
quote-grounding still keys off `page → page_texts[page]`); `escalate._single_group`
sends the whole field set over all pages in ONE call (only `max_fields_per_call`
can split it into a couple of same-page chunks). The `$ref` schema keeps ~200
fields ≈10 KB inline, under the Windows command-line limit, so the whole deal
fits one call. This is far faster than the band-batched fan-out and matches what
an analyst expects ("read the whole deal, give me the fields"); it implies
comprehensive (`force_assist`) escalation and bypasses the deterministic result
cache, so plans are always rebuilt complete. Merged LLM hits land on the
primary, and the multi-doc merge then collapses the group to one row by best
confidence per field. Set `one_call_per_deal: false` to restore the per-memo
band-batched path described next (byte-for-byte unchanged). `max_pages_per_deal`
bounds the combined payload.

The legacy **per-memo** path (`one_call_per_deal: false`): each memo whose
`EscalationPlan` has fields goes through the
...
IMAGE_TABLE and low-confidence SCANNED pages as ≤1080px PNGs) and exposes
per-page prompt blocks so calls can be page-scoped. A SCANNED page that OCRs
above `llm.ocr_text_min_confidence` is sent as **OCR text, not a page image**
(`prefer_ocr_text_over_image`): the Read-tool/vision path is slow (huge output,
re-done per call) and the OCR text is reliable; `ocr_hostile` (which routes AUTO
to opus) is then computed from the pages ACTUALLY sent as images, so a cleanly
scanned memo stays on sonnet. `schema_builder.py` compiles a byte-stable static
prompt + strict band-grouped JSON schema. The per-field answer shape is defined
ONCE under `$defs` and every field is a `$ref` to it, so a ~200-field schema is
~10 KB inline (not ~100 KB) and the whole escalate-everything set fits ONE call
under the Windows ~32 KB command-line limit. Schema **property keys are
sanitized** to the API's `^[A-Za-z0-9_.-]{1,64}$` rule (raw workbook headers like
`Total Invested Capital ($M)` and band names like `METHODOLOGY: MULTIPLE` are
rejected with HTTP 400) — the human header/band names live in the prompt's field
list, each annotated with its `{json key: …}`, and `response_key_map` reverses
the keys back to headers in `_flatten_response`. The wire prompt (`build_call_prompt`) is ordered
**cache-first** — stable rules + page payload, THEN the per-call field list — so
a memo's chunked calls and a group's retry tiers READ the (identical) page
payload from the server-side prompt cache instead of re-uploading it behind a
varying field-list prefix; the response-cache key still keys off the
field-set-derived `build_static_prompt` + payload hash + page scope.
...
`band_batched: false` reproduces the single-call-per-memo path. **Small-doc
collapse** (`llm.single_call_max_pages`, default 8): when the payload is at most
this many pages, `_build_groups` ignores band_batched and runs the whole document
+ all fields over one page set — band-batching's per-call page re-uploads only
pay off on large memos, so short client memos collapse to cheap/fast calls (the
model sees the whole doc at once); set it to 0 to always honor band_batched. The
field set is still chunked by `max_fields_per_call`: the response schema is passed
**inline** on the `claude` command line and Windows caps a command line at ~32 KB,
so a one-shot ~200-field schema (~40 KB) fails to launch (`[WinError 206]`). New
config keys (in `config.py` + `config.example.yaml`): `one_call_per_deal`,
`max_pages_per_deal`, `band_batched`,
`single_call_max_pages`, `band_relevance_floor`, `max_fields_per_call`,
`no_evidence_effort`.

**Adaptive page-locality batching** (`llm.adaptive_batching`, default on) is the
efficiency layer on top: one-call-per-band is wasteful when many bands point at
the SAME pages (a 3-page scanned memo had ~19 bands all on pages 1–3 → 19 calls,
each re-sending the same pages and paying per-call + rate-limit overhead).
`escalate._pack_by_pages` builds one unit per band, **merges units with identical
page-sets, then greedily combines page-sets whose union stays within
`adaptive_max_pages_per_call`** (and `max_fields_per_call`), chunking any over-cap
group. So a small document collapses to a handful of page-coherent calls (each
page-set read once, all its fields extracted together) while a large document
still fans out by where its data actually lives — the model still sees a focused
payload. The no-evidence sweep folds into a same-page call when one exists. Group
labels become `pages 1-3` etc. `adaptive_batching: false` = one call per band.
...
escalate to the next tier and surface leftover flags. **Manual mode forces ONE
model for everything** — no tier ladder, OCR-hostile docs included. Every
attempt lands in the audit record, `cost_ledger.jsonl`, and the Run Log.
`LLM_VERSION` 3.9.0 (cache key). NB: `claude --json-schema` takes the schema
...
- **`config.yaml`** — git-IGNORED, per-machine (seeded from `config.example.yaml`
  ...
  `llm` (workers, `budget_usd` default $25, routing, `allow_fable`), `gui`.
